ActivateApp::App.helpers do
  ...
  def next_action(current_action: uri.split('/').last)
    ...
    next_action ? redirect("/campaigns/#{@campaign.slug}?action=#{next_action}&name=#{@resource.try(:from_name)}&email=#{@resource.try(:from_email)}&address1=#{@resource.try(:from_address1)}&postcode=#{@resource.try(:from_postcode) or params[:postcode]}") : redirect("/campaigns/#{@campaign.slug}/thanks")  
  end
